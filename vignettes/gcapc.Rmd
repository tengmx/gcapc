...
customized by users.
  ...
  if only a subset of chromosomes are analyzed. This actually provides
  ...
  *bindingWidth* could be specified to limit searching in narrower
...
```{r bdwidth1}
bdw <- bindingWidth(cov,range=c(50L,150L),step=10L)
bdw
```
...
```{r gcbias}
gcb <- gcEffects(cov,bdw,samp=0.15,plot=TRUE)
```
...
peaks <- gcapcPeaks(cov,gcb,bdw,plot=TRUE)
peaks <- gcapcPeaks(cov,gcb,bdw,plot=TRUE,permute=20L)
peaks
```
